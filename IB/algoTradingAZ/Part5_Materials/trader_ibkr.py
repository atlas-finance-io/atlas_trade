...
window = 1
units = 1000
end_time = (dt.datetime.utcnow() + dt.timedelta(seconds=330)
            ).time()  # stop condition (5.5 mins from now)
sl_perc = 0.1
tp_perc = 0.1
...
def onBarUpdate(bars, hasNewBar):
    ...
    if bars[-1].date > last_bar:
        ...
        # Data Processing
        df = pd.DataFrame(
            bars)[["date", "open", "high", "low", "close"]].iloc[:-1]
...
def execute_trade(target):
    ...
    if target > 0:  # GOING LONG
        if current_pos == 0:  # from NEUTRAL
            go_long_short(side="BUY", target=target,
        ...
        if current_pos == 0:  # from NEUTRAL
            go_long_short(side="SELL", target=abs(target),
                          sl_price=sl_price, tp_price=tp_price)
...
# NEW Go Long/Short starting from Neutral posistion
def go_long_short(side, target, sl_price, tp_price):
...
def trade_reporting():
    ...
    fill_df = util.df([fs.execution for fs in ib.fills()])[
        ["execId", "time", "side", "shares", "avgPrice"]].set_index("execId")
    profit_df = util.df([fs.commissionReport for fs in ib.fills()])[
        ["execId", "realizedPNL"]].set_index("execId")
    report = pd.concat([fill_df, profit_df], axis=1).set_index(
        "time").loc[session_start:]
    report = report.groupby(["time", "side"]).agg(
        {"shares": "sum", "avgPrice": "mean", "realizedPNL": "sum"}).reset_index().set_index("time")
...
def stop_session():
    ...
    while True:
        ib.sleep(5)
        try:
            current_pos = [pos.position for pos in ib.positions(
            ) if pos.contract.conId == conID][0]
        except:
            current_pos = 0
